...
export default class SchemaRegistryContainer extends Component {
  ...
  handleEditDescription() {
    if (this.state.editDescription) {
      ...
      return SchemaREST.postSchema({
        body: JSON.stringify(this.state.currentSchema)
      });
    } else {
  ...
  onToggle(node, toggled) {
    if (this.state.currentSchema) {
      this.state.currentSchema.active = false;
    }
    node.active = true;
    let schemaName = this.schemaName || this.state.currentSchema.name;
    let schemaObj = _.find(this.state.schemaData, (obj) => {
      return obj.name === schemaName;
    });
  ...
  showSchema() {
    ...
    if (schemaData.length) {
      if (this.schemaName) {
        ...
        this.onToggle(schemaData[0], true);
      }
    }
  }
  handleSaveVersion() {
                ...
                <div className="row">
                  <div className="col-sm-3">
                    <div className="schema-tree">
                      <Treebeard data={this.state.schemaData} onToggle={this.onToggle.bind(this)} style={this.style} decorators={{
                        Toggle: (props) => {
                          if (props.node.children && props.node.children.length > 0) {
                            return (
                              <i className={props.node.toggled
                                ? "fa fa-caret-down"
                                : "fa fa-caret-right"}></i>
                            );
                          } else {
                        ...
                        Header: (props) => {
                          if (props.node.children) {
                            return (
                              <span>
                                <i className={props.node.toggled && props.node.children.length > 0
                                  ? 'fa fa-folder-open-o'
                                  : 'fa fa-folder-o'}></i>
                              ...
                              <div>
                                ...
                                <div>
                                  <div className="schema-content">
                                    ...
                                    <ul className="schema-changelog"></ul>
                                  </div>
                                </div>
                              </div>
                            )
                    ...
                    <div className="schema-sidebar">
                      <div className="higlighted-section">
                        {schema.children.length === 1
                          ? <p>
                              <strong>There is 1 version of this Schema</strong>
                            </p>
                          : <p>
                            <strong>There are {schema.children.length}
                              versions of this Schema</strong>
                          </p>
}
                        <ul className="schema-versions">
                          {schema.children.map((v, i) => {
                            return (
                              <li className="clearfix" key={i} onClick={this.selectVersion.bind(this, v)}>
                                <i className="fa fa-folder-o pull-left"></i>
                                <p>
                                  <strong>Version {v.id}</strong><br/>
                        ...
                        {schema.children.length > 1
                          ? <p>
                              <a href="javascript:void(0);">COMPARE VERSIONS</a>
                            </p>
                          : null}
                        {showSchema
                          ? <p>
                              <a href="javascript:void(0);" onClick={this.handleAddVersion.bind(this)}>ADD VERSION</a>
                            </p>
                          : ''}
                      </div>
                    </div>
